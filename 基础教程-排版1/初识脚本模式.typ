...
四的三次方是 #calc.pow(4, 3)。这里进入脚本模式并调用了内置的计算函数。

函数的多种写法具有等价性，下面给出不同写法但效果相同的演示：
...
#highlight(fill: orange)[具名传参]，这里使用了 highlight 函数的具名传参。
...
此处显示为“Figure 1”的原因是，没有把文档的默认语言调整为中文。
...
自此，基本知识已经基本掌握了，已经可以编写出比 Markdown 更加复杂的文档了。
